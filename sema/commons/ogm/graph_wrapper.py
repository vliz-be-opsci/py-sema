...
class GraphWrapper:
    ...
    def set_jsonld_context(self, context) -> None:
        ...
        self.graph._jsonld_context_repr = context  # Context.to_dict() doesn't preserve original representation # noqa: E501
    ...
    def update_node(
        ...
        if properties:
            for predicate_template, object_tmp in properties.items():
                predicate = TermBuilder(
                    predicate_template, graph=self.graph
                ).build()
                object_templates = self._listify(object_tmp)
                for object_template in object_templates:
                    object = TermBuilder(
                        object_template, graph=self.graph
                    ).build()
    ...
    def create_node(
        ...
        properties: (
            dict[URIRef | str, URIRef | Literal | str | list | None] | None
        ) = None,
        kind: str = "blank",
        typed: bool = True,
    ) -> URIRef | BNode:
        ...
        a: URIRef, str, list of these, or None. "a" is shorthand for "type" in
        RDF terminology.
        kind: If "blank", the identifier will be a BNode (blank node). If "iri"
        or "relative", the identifier will be a URIRef.
        typed: If True, a type declaration must be added to the node. Then, if
        a=None, the type declaration will be "rdfs:Resource".
        """
        if kind == "iri":
            subject_template = identifier or f"<{uuid4()}>"
            subject = TermBuilder(
                template=subject_template, graph=self.graph
            ).build()
        elif kind == "relative":
        ...
        self.update_node(
            identifier=subject, a=a, label=label, properties=properties
        )
