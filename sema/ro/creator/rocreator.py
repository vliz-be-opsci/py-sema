...
class ROCreator(ObjectGraphMapper):
    def __init__(
        ...
        self.rocrate_metadata_name = (
            rocrate_metadata_name or "ro-crate-metadata.json"
        )
        self.force = force
    ...
    def process(self, *args, **kwargs) -> None:
        ...
        if destination.exists() and not self.force:
            logger.warning(
                f"File {destination} already exists. Use --force or force=True"
                " to overwrite."
            )
            return
        self.serialize(
            destination, format="application/ld+json", *args, **kwargs
        )
