...
def assertFormat(content: any, type_name: str, expected: str, quote: str | None = "'") -> None:
    format = xsd_fmt(content, type_name, quote)
    assert format == expected, f"unexpected {type_name} format for {content} using {quote}:  {format} != {expected}"
...
def test_rigid_bool() -> None:
    ...
    with pytest.raises(AssertionError):
        ...
        assert False, "None should not be accepted as bool value"

    # [] conversion (empty or not) required before formatting
    with pytest.raises(AssertionError):
        xsd_fmt([], "xsd:boolean")
        assert False, "[] should not be accepted as bool value"
...
def test_rigid_int() -> None:
    ...
    with pytest.raises(AssertionError):
        ...
        assert False, "leading zero's should not be accepted in strings for int value"

    # floats should not be accepted
    with pytest.raises(ValueError):
        xsd_fmt(1.0, "xsd:integer")
        assert False, "floats should not be accepted as int value"

    # floats disguised as strings should not be accepted
    with pytest.raises(ValueError):
        xsd_fmt("1.0", "xsd:integer")
        assert False, "floats disguised as strings should not be accepted as int value"
...
@pytest.mark.parametrize("case", [
    ...
    ((datetime.fromisoformat("2021-09-30T16:25:50+02:00").date(), "'2021-09-30'^^xsd:date")),
])
...
def test_rigid_date() -> None:
    with pytest.raises(AssertionError, match="use datetime format for datetime values"):
        xsd_fmt(datetime.fromisoformat("2021-09-30T16:25:50+02:00"), "date")


@pytest.mark.parametrize("case", [
    (("2021-09-30T16:25:50+02:00", "'2021-09-30T16:25:50+02:00'^^xsd:dateTime")),
    ((datetime.fromisoformat("2021-09-30T16:25:50+02:00"), "'2021-09-30T16:25:50+02:00'^^xsd:dateTime")),
])
...
@pytest.mark.parametrize("case", [
    (("https://example.org/for/testing", "'https://example.org/for/testing'^^xsd:anyURI")),
    (("https://example.org/for/[testing]", "'https://example.org/for/%5Btesting%5D'^^xsd:anyURI")),
])
...
@pytest.mark.parametrize("case", [
    ...
    ((">\n<", '""">\n<"""^^xsd:string', '"')),
    (("Hello!", "'Hello!'^^xsd:string")),
    (("ceci n'est pas une texte", "'ceci n\\'est pas une texte'^^xsd:string")),
...
def test_urit_fn() -> None:
    ...
    assert isinstance(uritexpand_fmt, Callable), "uritexpand_fmt function not callable"
...
def test_regexreplace_fn() -> None:
    ...
    assert isinstance(regexreplace_fmt, Callable), "regexreplace_fmt function not callable"
...
def test_mapbuild_fn() -> None:
    ...
    assert isinstance(map_build_fmt, Callable), "map_build_fmt function not callable"


def test_mapbuild() -> None:
    map_expects = dict(a=5, b=6, c=7)
    ...
    for origin in map_expects.keys():
        ...
        assert record["to-field"] == map_expects[origin], "map not applied correctly"