...
def test_resolve_loader() -> None:
    ...
    out = yaml.safe_load(yml_text, Loader=resolving_loader)
    assert out["value"] == "1.2"

    # 4. when refering to unknown key we get no resolution
    #    note that a keyerror will be logged (but not tested here)
    err_text = "value: !resolve '{a}.{c}'"
    out = yaml.safe_load(err_text, Loader=resolving_loader)
