...
def test_cli(capfd):
    ...
    assert (TEMPLATES_FOLDER / name).exists(), f"template {name=} does not exist in {TEMPLATES_FOLDER=}"

    def make_multipart(argname: str, valdict: Dict[str, Any]):
        return ' '.join(f"--{argname} {k} {v}" for k, v in valdict.items())

    setnames: Dict[str, str] = dict(_="data.csv", cities="data_cities/", movies="data_movies.xml")
    setpaths: Dict[str, Path] = {k: (DATA_FOLDER / v) for k,v in setnames.items()}
    ...
    cli_line = f"--logconf {logconf_path} --templates {TEMPLATES_FOLDER} --name {name} {setpart} {varpart} --output - "
