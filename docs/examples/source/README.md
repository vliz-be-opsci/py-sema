...
This examples set is organized by submodule folders (not standalone root Python scripts).

### 1) SPARQL Query (`examples/query/`)
...
### 2) Task Orchestration (`examples/bench-orchestration/`)
...
### 3) RDF Harvesting (`examples/harvest/`)
...
**Run:**
```bash
type examples\harvest\README.md
```

### 4) RDF Discovery (`examples/discovery/`)
...
**Run:**
```bash
type examples\discovery\README.md
```

### 5) Template-Based Generation (`examples/subyt/`)
...
- `examples/subyt/` - Convert structured data to RDF
- `examples/subyt/` - CLI-focused sema.subyt examples and files
- `examples/discovery/` - Discover and retrieve RDF from URIs

### Data Processing
- `examples/query/` - CLI-focused sema.query examples
- `examples/bench-orchestration/` - Validate RDF with SHACL as part of workflows

### Data Management
- `examples/harvest/` - Harvest RDF from sources

### Orchestration & Packaging
- `examples/bench-orchestration/` - Orchestrate multiple services
...
1. **Start with `examples/query/`** - Understand basic RDF querying
2. **Try `examples/subyt/`** - Generate RDF from structured data
3. **Review `examples/discovery/`** - Discover RDF from web resources
4. **Try `examples/harvest/`** - Collect RDF data from sources
5. **Learn `examples/bench-orchestration/`** - Orchestrate complex workflows and SHACL checks
6. **Explore `examples/ro/`** - Package research outputs
7. **Check the module READMEs** - Deep dive into specific modules
8. **Review test files** - See real-world usage in `tests/` directory
...
1. Create a well-commented example folder (and scripts where relevant) for a specific use case
